...
export const VISUAL_TOKENS = {
  ...
  colors: {
    ...
    /** 炎上・鎮火・介入リアクションの DOM/Pixi 共通色（RI-142）。 */
    boardEffects: {
      fireCore: '#fff8c0',
      fireMid: '#ff7a2f',
      fireEdge: '#c02000',
      extinguishCore: '#ffffff',
  ...
  dimensions: {
    sprint: {
      ...
      /** 一時演出の寿命・共有寸法・上限。設計 px と ms で DOM/Pixi を揃える（RI-142）。 */
      boardEffects: {
        budget: 20,
        lingerMs: 80,
        spread: { size: 14, durationMs: 550 },
        extinguish: { size: 36, firefightSize: 52, durationMs: 500 },
        ignite: { size: 40, durationMs: 450 },
        sweep: { size: 16, durationMs: 480, staggerMs: 90, trailLength: 34 },
        split: { badgeWidth: 44, badgeHeight: 18, shardSize: 10, durationMs: 520 },
        firefight: { size: 48, burstSize: 52, durationMs: 550 },
        assignDash: { length: 36, width: 4, durationMs: 420 },
        fullPulse: { durationMs: 600 },
        aura: {
          pulsePeriodMs: 2400,
          minAlpha: 0.12,
          elapsedAlpha: 0.18,
...
export function visualTokenCssVariables(): Readonly<Record<string, string>> {
  ...
  return {
    ...
    '--visual-board-effect-spread-size': `${(dimensions.sprint.boardEffects.spread.size / spaces.sprint.w) * 100}%`,
    '--visual-board-effect-extinguish-size': `${(dimensions.sprint.boardEffects.extinguish.size / spaces.sprint.w) * 100}%`,
    '--visual-board-effect-extinguish-firefight-size': `${(dimensions.sprint.boardEffects.extinguish.firefightSize / spaces.sprint.w) * 100}%`,
    '--visual-board-effect-ignite-size': `${(dimensions.sprint.boardEffects.ignite.size / spaces.sprint.w) * 100}%`,
    '--visual-board-effect-sweep-size': `${(dimensions.sprint.boardEffects.sweep.size / spaces.sprint.w) * 100}%`,
    '--visual-board-effect-split-shard-size': `${(dimensions.sprint.boardEffects.split.shardSize / spaces.sprint.w) * 100}%`,
    '--visual-board-effect-firefight-size': `${(dimensions.sprint.boardEffects.firefight.size / spaces.sprint.w) * 100}%`,
    ...
    '--visual-color-effect-fire-edge': colors.boardEffects.fireEdge,
    '--visual-color-effect-extinguish-core': colors.boardEffects.extinguishCore,
    '--visual-color-effect-extinguish-wash': colors.boardEffects.extinguishWash,
    '--visual-color-effect-sweep-core': colors.boardEffects.sweepCore,
    '--visual-color-effect-sweep-mid': colors.boardEffects.sweepMid,
    '--visual-color-effect-sweep-edge': colors.boardEffects.sweepEdge,
    '--visual-color-effect-rework-core': colors.boardEffects.reworkCore,
    '--visual-color-effect-rework-mid': colors.boardEffects.reworkMid,
    '--visual-color-effect-rework-edge': colors.boardEffects.reworkEdge,
    '--visual-color-effect-split-panel': colors.boardEffects.splitPanel,
    '--visual-color-effect-split-text': colors.boardEffects.splitText,
    '--visual-color-effect-split-shard': colors.boardEffects.splitShard,
    '--visual-color-effect-firefight': colors.boardEffects.firefight,
