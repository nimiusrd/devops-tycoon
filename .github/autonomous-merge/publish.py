...
def main() -> int:
    ...
    try:
        if not args.report_dir.is_dir():
            raise PublishError("observation directory is missing; run observation first")
        api = GitHub(args.repository)
        ensure_labels(api)
        # 全体収集の失敗で対象不明の場合、既存のopen PRラベルは残す。
        failed = (args.report_dir / "collection-error.json").exists()
        for path in sorted(args.report_dir.glob("pr-*.json")):
            try:
                number = int(path.stem.removeprefix("pr-"))
                if number <= 0:
                    raise PublishError("PR number must be positive")
                report = json.loads(path.read_text())
                print(f"PR #{number}: {publish_pr(api, number, report)}", flush=True)
