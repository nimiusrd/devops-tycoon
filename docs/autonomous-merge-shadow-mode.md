...
| 鮮度 | head / base / test mergeのSHA、PR更新時刻、収集前後の状態 | 収集中に変化した場合は情報不足。古いSHAのCIや承認は流用しない |
...
| `INSUFFICIENT_DATA` | API失敗、権限不足、ページング上限、必須情報欠落、収集中のPR変更など |
...
これらはAPIを複数回読んだ観測であり、トランザクションでもロックでもありません。
