...
| RI-60 | 初見向けオンボーディング(チュートリアル/ヘルプ) | 中 | 完了 | — | 第4 / 6 / 7 |
...
#### RI-60 初見向けオンボーディング(チュートリアル/ヘルプ) — 優先度:中 / 完了

**完了**: タイトルの「遊び方」ヘルプ（`HowToPlayScreen`）、初回スプリントの段階ガイド
（介入バー→レビュー渋滞→コンボ）、`MetaState.seenTutorial` 永続化、`?tutorial=`
（`1`/`force`/`help`/`off`）フックを UI 層のみで実装。`markTutorialSeen()` で表示済みを保存。
Vitest: `tests/unit/tutorial.test.ts`。E2E: `tests/e2e/tutorial.spec.ts`。
