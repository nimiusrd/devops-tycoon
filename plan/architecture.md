...
- 正本は二系統である。[balance-ssot-plan.md](./balance-ssot-plan.md)の分類に従い、移行対象の共通パラメータ（工程、介入、ラン進行、KPI、ペーシングなど）だけが型付き分割レジストリ（`src/data/balance/`）を正本とする。カード、イベント、レリック、難易度、試練、アンロックなどの既存コンテンツは`src/data/`の定義を正本とし、値をレジストリへ複製しない。SSoT移行（RI-104）は完了済みである。
