 ...
 *
 * 難易度と試練の選択ロジックは維持しつつ、AI 時代の開発組織を指揮する
 * 戦略ゲームとして情報の優先順位と視覚的フィードバックを強化する。
 */
...
export function TitleScreen({
  ...
  const scoreMultiplier = useMemo(
    () =>
      TRIAL_DEFS.filter((trial) => trials.includes(trial.id)).reduce(
        (multiplier, trial) => multiplier * trial.scoreMul,
        1,
      ),
    [trials],
  );
    ...
    <div className="title-screen title-command" data-testid="title">
      ...
      <header className="title-topbar">
        <div className="title-brand">
          <span className="title-brand-mark">D<span>O</span></span>
          <span><b>DEVOPS TYCOON</b><small>AI ERA // ORGANIZATION SIM</small></span>
        </div>
        <div className="title-season"><small>SEASON 01</small><b>THE AGENTIC SHIFT</b></div>
        <div className="title-career">
          <span><small>META RANK</small><b>{meta.points}</b></span>
          <span><small>PERSONAL BEST</small><b>{meta.bestScore}</b></span>
        </div>
      </header>

      <div className="title-content">
        <header className="title-head">
          <p className="title-eyebrow"><span /> NEW QUARTER // Q3 2026</p>
          <h1 className="title-logo">組織を選び、<br /><em>AI時代を生き残れ。</em></h1>
          <p className="title-sub">開発文化、技術的負債、そして人間関係。<br />90日でチームを変革する組織シミュレーション。</p>
          <div className="title-meta">
            <span className="pill" data-testid="seed">◈ SEED <b>{seed}</b></span>
        ...
        <main className="title-deck">
          <div className="title-deck-head">
            <div><span className="title-step">01</span><p><b>CHOOSE YOUR ORGANIZATION</b><small>開始する組織の成熟度を選択</small></p></div>
            <span className="title-ready"><i /> SYSTEM READY</span>
          </div>
            ...
            <div className="difficulty-grid">
              {DIFFICULTY_ORDER.map((id) => {
                ...
                return (
                  ...
                  >
                    <span className="difficulty-kicker"><b>{DIFFICULTY_TAG[id]}</b><i>{unlocked ? 'SELECT' : 'LOCKED'}</i></span>
                    <span className="difficulty-name">{def.label}</span>
                    <span className="difficulty-desc">
                      {unlocked ? def.description : '未解放 — 下位難易度のクリアが必要'}
                    </span>
                    <span className="difficulty-action">{unlocked ? '組織プロファイルを選択 →' : '条件未達'}</span>
                  </button>
                );
              })}
            </div>
          ...
          <section className="title-section title-trial-section">
            <div className="title-section-copy"><span className="title-step">02</span><p><b>ADD CHALLENGES</b><small>任意の制約でスコア倍率を上げる</small></p></div>
            <div className="trial-row">
              {TRIAL_DEFS.map((trial) => (
                ...
                >
                  <i>{trials.includes(trial.id) ? '×' : '+'}</i>{trial.label}
          ...
          {onStartDaily && (
            <section className="title-launch-row" data-testid="daily-run-section">
              <div className="title-daily">
                <span>DAILY RUN</span><b>全員同じ条件で競う</b>
                <small>UTC {today}・{dailyRecord ? `今日のベスト ${dailyRecord.bestScore} pt` : 'まだ今日の記録はありません'}</small>
                <button type="button" data-testid="start-daily-run" onClick={onStartDaily}>本日のデイリー →</button>
              </div>
              <div className="title-mission">
                <small>MISSION PROFILE</small>
                <b>{DIFFICULTY_TAG[difficulty]} / {trials.length} MODS</b>
                <span>FINAL MULTIPLIER <strong>×{scoreMultiplier.toFixed(2)}</strong></span>
              </div>
              <button
                type="button"
              ...
              >
                <span><small>BEGIN THE QUARTER</small>四半期を始める</span><i>→</i>
              </button>
            </section>
          )}
          ...
          {!onStartDaily && (
            <div className="title-actions">
              <button type="button" className="btn btn-primary btn-lg" data-testid="start-run" onClick={() => onStart(difficulty, trials)}>
                四半期を始める →
              </button>
            </div>
          )}
        ...
        {meta.achievements.length > 0 && (
          <section className="title-achievements">
            ...
            {meta.achievements.slice(0, 3).map((achievement) => <b key={achievement}>◇ {ACHIEVEMENT_LABEL[achievement] ?? achievement}</b>)}
          </section>
        )}
        ...
        <footer className="title-footer">
          ...
          <nav>
            {onOpenMetaShop && <button type="button" data-testid="open-meta-shop" onClick={onOpenMetaShop}>研修ツール</button>}
            {onOpenAchievements && <button type="button" data-testid="open-achievements" onClick={onOpenAchievements}>実績コレクション</button>}
          </nav>
        </footer>
      </div>
    </div>
