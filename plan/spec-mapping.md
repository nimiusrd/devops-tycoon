...
| [17](../SPEC.md#17-メタ進行とアンロック) | メタ進行とアンロック | `src/state/meta.ts`, `src/state/metaPersistence.ts`, `src/data/unlocks.ts`, `src/ui/MetaShopScreen.tsx`, `AchievementCollectionScreen.tsx` / `tests/unit/meta.test.ts`, `metaPersistence.test.ts`, `meta-unlock-run.test.ts`, `unlocks.test.ts` | ✅ IndexedDB によるカード/レリックの永続解放・旧 localStorage 移行・メタショップ・実績閲覧は実装。開始時の組織は難易度（§16）で選定し、メタ解放プリセットはスコープ外（[remaining-issues.md](./remaining-issues.md) RI-25 完了）。メンバー／トレイト／開始キット等の解放拡張もスコープ外（RI-24 完了） |
...
| [23](../SPEC.md#23-拡張案) | 拡張案 | デイリーラン＋「なぜ燃えたか」解説ログ（RI-34′）を実装。他の拡張案（GitHub 連携・図鑑等）は未着手 | 🟡 |
...
| 拡張案（デイリーラン・なぜ燃えたか以外: GitHub 連携・対抗ランキング ほか） | [23](../SPEC.md#23-拡張案) | 一部着手（RI-34′ 完了）。本体は保留 | [remaining-issues.md](./remaining-issues.md) RI-34 / RI-34′ |
