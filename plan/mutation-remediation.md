...
再ベースラインでは、文書切替より先に旧エピック Issue と旧エピック配下の **全 open サブイシュー**を置換／不要理由付きで close する。途中作成 Issue の再利用は、本文の run URL / `headSha` が一致するときだけ（番号にかかわらず検索する。未紐づけの作成済み単位 Issue も二重作成しない）。
