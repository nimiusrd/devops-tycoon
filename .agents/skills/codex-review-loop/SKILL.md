...
`@codex review` を投稿したあとは、Codex の応答を待つ。指摘があればコメントし、なければそのコメントに 👍 する。指摘が出たら対応して再度 `@codex review` する。コード変更なしで dismiss して resolve しただけの場合は、同じ HEAD でも再依頼してよい。
...
- 最新 HEAD に対する Codex 完了がある。`@codex review` 投稿済みならそのコメントへの 👍、またはその依頼で出た指摘をすべて対応・resolve 済み。未投稿なら初回自動レビューの完了でよい
