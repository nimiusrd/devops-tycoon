...
1. **既定テンポの減速**: `useRun` の実時間→sim 時間の比を変える。SPEC §3.1 の目安
   （スプリント 60〜120 秒）を現状の tick 数分布（36〜204 tick・平均 104 tick）のまま満たすには
   **1 tick / 約600〜1150ms 実時間**が必要（例: 150〜250ms 程度の減速では平均 16〜26 秒にしか
   ならず目安に届かない）。sim 層・seed 決定論・モンテカルロ（tick 数ベース）には非干渉で、
   E2E / gallery も `step()` 直接駆動のため影響なし（要確認）。ただし 1 tick が 1 秒近くなると
   盤面の粒がカクついて見える恐れがあり、その場合は ④ の tick 数増とテンポ減速の**併用**で
   「tick は細かく・総時間は長く」に振り直す（①単独で足りるかは実装時にプレイフィールで判断）。
...
4. **スプリント長自体の延伸（tick 数増）**: `taskCount` / intake ペースの変更でスプリントの
   tick 数を増やし、1 tick あたりの実時間を上げすぎずに §3.1 の総時間を満たす。バランス全体・
   MC 許容レンジ・終端 seed（E2E_TERMINAL_*）に波及するためコストは大きいが、①の減速幅
   （1 tick / 約1秒）が体感的に許容できない場合は①との併用が本命になる。
