...
2. **衝突回避の読み取りのみ:** [`plan/remaining-issues.md`](../../../plan/remaining-issues.md) や他ドキュメントに残る既存 `RI-(\d+)`（一般バックログとの番号衝突を避ける。ここに番号を予約・追記して採番を確定させない）
...
4. **作成直後に競合を再確認する。** 同じタイトル先頭 `[RI-{N}]` のエピック Issue が自分以外にもあれば、並列採番が衝突している。自分の Issue 番号が大きい方を、改めて次番号を取り直してタイトル変更する（単位 Issue を切る前に確定させる）。
...
5. エピック Issue がすでに close で、かつ不足単位を足す必要があるときだけ、確認後にエピック Issue を **reopen** する（履歴エピックの書式修正だけでは reopen しない）。
...
- 親 Issue 作成直後の同 ID 競合を再確認せず、単位 Issue 作成へ進む
...
- 完了済みエピックへ不足 Issue を足すとき、親 Issue を reopen しない
