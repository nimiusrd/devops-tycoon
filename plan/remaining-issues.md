...
| RI-58 | ラン途中セーブと復帰(ランステート永続化) | 中 | 完了 | — | 第17 / 22 |
...
#### RI-58 ラン途中セーブと復帰(ランステート永続化) — 優先度:中 / 完了

**完了**: 保存形式は `RunEngine.exportPersistState()` / `hydratePersistState()` によるスナップショット
直列化（案 A）を採用。seed＋入力ログ再生（案 B）は RI-61 へ切り出し。IndexedDB
（DB `devops-tycoon` v2 / store `runSave`）へフェーズ遷移時のみ保存し、スプリント tick 中は
直前セーブを維持。`schemaVersion` 不一致・破損は破棄。タイトルに「続きから」導線と
`tests/e2e/run-persistence.spec.ts` のリロード復帰を追加。
