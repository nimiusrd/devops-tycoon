...
| 種別 | 形式 | 管理場所 | 例 |
| --- | --- | --- | --- |
| エピック（ベースライン） | `RI-{N}` | **GitHub Issue**（タイトル `[RI-{N}]`） | `RI-N` |
| 実装単位 | `RI-{N}-{GROUP}{SEQ}` | **GitHub Issue**（1 Issue = 1PR。エピックのサブイシュー） | `RI-N-A1` |

規則:

- 採番・進捗の正本は GitHub Issue。`remaining-issues.md` に予約行や番号台帳を置かない。
- `{N}` は open/closed のエピック Issue タイトルから見た既存 `RI-(\d+)` の最大+1（欠番再利用なし）。一般バックログとの衝突回避のため文書上の既存 ID は読み取るだけ（例示の `RI-N` やプレースホルダーは採番対象にしない）。フルシャード run の再ベースラインごとに新規エピック。親 Issue 作成が番号予約であり、同 ID 競合は「次番号取得 → タイトル変更 → 再確認」を競合解消まで繰り返す。現行より古い run は明示確認なしで新ベースラインにしない。
- `{GROUP}` は大文字1文字 `A`–`Z`（A が最高優先）。`{SEQ}` はグループ内 1, 2, 3…（ゼロ埋め・欠番再利用なし）。
- **1実装単位 = 1 Issue = 1PR。** Issue / PR タイトル先頭に ID（例: `[RI-N-A1] industry の mutation 耐性を上げる`）。
...
サブイシューがすべて closed になったらエピック Issue も close する。親が close でも必要なサブイシューが1件でも open ならエピックを reopen する。  
...
- `engine.ts` の `step` / `playCard` は `sim-run-engine-e`、`resolveSprint`（baseline 完走）は `sim-run-engine-g` として切り離し、mutant 予算 160。`vitest.mutation.config.ts` は `tests/unit/ui/sprintTempo.test.ts` を除外する（instrument した `cards.ts` で F-4/F-5 の beforeAll が 300s を超え dry-run が落ちるため）。
