...
| [18（描画基盤）](../SPEC.md#18-視覚表現) | 視覚表現の WebGL 化 | `src/render/adapters/pixiOrgRenderer.ts`, `pixiDeptRenderer.ts`, `pixiBoardRenderer.ts`, `selectRenderer.ts`, `src/ui/OrgPixiField.tsx`, `DeptPixiField.tsx`, `BoardPixiField.tsx` / `tests/e2e/*-pixi-visual.spec.ts`（`?renderer=pixi` で opt-in） | ✅ 全社・部署・現場盤面を Pixi 化（既定は DOM/SVG。スプライト化は RI-07、カメラ演出は RI-04） |
