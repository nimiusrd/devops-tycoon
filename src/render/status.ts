...
export const SENIOR_HP_HELP =
  'メンバー個別のスタミナとは別の抽象値です。炎上は複数炎上やタイマーが短いときだけ緊急対応で消し、余裕のある先消しは避けます。アンドンは流入を止めてキューを捌く猶予を作り、AIスロットルは新規タスクをAIなしにします。未熟なワークフローでは点火・手戻りを抑えますが、前提度が高く成熟していると工程ずれで手戻りが増えることがあります。休息で戻します。';
