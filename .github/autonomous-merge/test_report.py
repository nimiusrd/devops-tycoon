...
from test_support import HEAD, facts, policy


def result(data):
    return {
        "schema_version": 2, "mode": "shadow", "decision": "WAITING",
        "conditions": [{"name": "example", "status": "waiting", "detail": "reason"}],
        "observations": data, "policy": policy(), "policy_sha256": "fingerprint",
    }
...
class ReportTests(unittest.TestCase):
    ...
    def test_missing_null_and_empty_diagnostics_keep_distinct_meanings(self):
        for diagnostic, expected in (
            ...
            ({"observation_changes": [{"group": "pr", "field": "draft", "before": False, "after": True}]}, "draft"),
        ):
