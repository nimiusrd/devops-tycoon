...
さらに、`expect`のshadow判定はtest callbackだけでなく、それを包含するsuite callbackのlexical scopeまで追跡します。PlaywrightのPNG snapshotは、所有specの非disabledな実行対象test callback内にある`toHaveScreenshot()`からの参照だけを検証として受理し、未使用helperの呼び出しは除外します。通常・parameterized testは、Vitest／Playwrightまたは既知のfixtureからimportされた未shadowの`test`・`it`・`specify` bindingに限って認識します。
