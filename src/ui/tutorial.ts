...
export const TUTORIAL_STEPS: readonly TutorialStep[] = [
  {
    ...
    title: '介入バー',
    body: 'マネジメント集中力を使って現場へ介入します。緊急対応は複数炎上や延焼直前（タイマーが短いとき）だけが本命で、余裕のある先消しは高コストです。アンドンは既存キューを捌く猶予、AIスロットルは新規タスクをAIなしにします。ワークフローが未熟なら点火・手戻りを抑えますが、前提度が高く成熟しているときは工程ずれで手戻りが増えることがあります。品質・PR分割は手戻り率だけを下げます。ペアレビューは詰まったPRを処理しつつAI Literacyを上げ、依存度が高くてもリテラシーが低いままだと敗北するので早めに使いましょう。編成で review や稼働人数を増やすと、レビュー単価とシニア消耗が薄まり燃え尽きにくくなります。',
  },
  {
    id: 'senior-hp',
    targetTestId: 'hud-seniorHp',
    title: 'シニア体力',
    body: 'シニア体力はメンバー個別のスタミナとは別の抽象値です。尽きるとシニア燃え尽きで敗北します。炎上の自動鎮火は大きく削りますが、緊急対応も余裕のある先消しでは高コストでコンボが切れます。複数炎上やタイマーが短いときだけ打ち、アンドンは流入を止めてキューを捌く猶予を作り、AIスロットルは新規タスクをAIなしにします。未熟なワークフローでは点火・手戻りを抑えますが、前提度が高く成熟していると工程ずれで手戻りが増えることがあります。休息で体力を戻します。',
  },
  {
    id: 'jam-meter',
