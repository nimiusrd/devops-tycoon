...
class ContractTests(unittest.TestCase):
    def test_valid_boundary_values_are_preserved(self):
        for validate, arguments, expected in (
            (integer, (0, "count"), 0), (integer, (10**30, "count"), 10**30),
            (boolean, (False, "stable"), False), (boolean, (True, "stable"), True),
            (string, ("日本語", "name"), "日本語"),
            (sha, ("a" * 40,), "a" * 40),
        ):
    ...
    def test_invalid_values_keep_the_existing_error_messages(self):
        for validate, values, suffix, message in (
            (integer, [None, True, -1, 1.0, "0", []], ("count",), "count: non-negative integer required"),
