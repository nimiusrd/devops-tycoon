...
6. **エピック Issue と必要なサブイシューが揃ったことを確認してから**文書を切り替える。`gh issue create` / サブイシュー紐づけが使えずユーザーへ手順だけ渡す場合は、ここで中断する（旧エピックの完了扱いや計画の新エピック切替は行わない。作成確認後に再開）。
...
9. 直前のミューテーション改善エピック（未着手・進行中）があれば文書上 **完了** にし、完了要約へ「後続ベースライン `RI-XX` に置換。未消化は新 Issue へ引き継ぎ」と書く。あわせて **旧エピック Issue と、引き継いだ旧サブイシューをすべて close** する（コメント例: `RI-XX へ置換。未消化は #<新Issue> へ引き継ぎ`）。
