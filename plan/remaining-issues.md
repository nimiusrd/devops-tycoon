...
1. **既定テンポの減速**: `useRun` の実時間→sim 時間の比を変える（例: 1 tick / 150〜250ms 実時間）。
   sim 層・seed 決定論・モンテカルロ（tick 数ベース）には非干渉で、E2E / gallery も `step()` 直接
   駆動のため影響なし（要確認）。最小差分で効く。
...
4. **スプリント長自体の延伸**: `taskCount` / intake ペースの変更はバランス全体・MC 許容レンジ・
   終端 seed（E2E_TERMINAL_*）に波及するため、①〜③で足りない場合の別判断とする。
