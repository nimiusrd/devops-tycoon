...
再ベースラインでは、文書切替より先に **旧エピック Issue 自体**と配下の **全 open サブイシュー**を置換／不要理由付きで close する。未消化を引き継ぐときは、関連する open PR の ID / `Fixes`・`Closes` も新 Issue へ付け替える。途中作成 Issue の再利用は、本文の run URL / `headSha` が一致するときだけ（番号にかかわらず検索する。未紐づけの作成済み単位 Issue も二重作成しない）。
