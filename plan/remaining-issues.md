...
| RI-28 | 四半期レビュー評価→メタ報酬の接続（′可視化完了 / ″解放・途中付与は保留） | 低 | 完了 | — | 第8 / 17 |
...
**完了（コア + RI-28′）**: `reviewHistory` の outcome を `applyRunReward` へ接続済み
...
**保留（RI-28″）**: outcome を解放条件ゲートにする、または四半期ごとのラン中即時ポイント付与は
設計選択で変更範囲が大きいため未着手。
