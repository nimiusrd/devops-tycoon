...
- 静的索引の追記・Baseline 補完・書式修正
- **不足している単位ファイルだけ追加**する。既存の `plan/mutation-units/<ID>.md` は上書きしない（`状態` / `After:` など進捗の正本を消さない）
...
- **同じ run**: 既存エピックを再利用し、静的索引の追記・Baseline 補完・書式修正、**不足単位ファイルの追加のみ**行う（既存単位ファイルは上書きしない。新エピック採番なし）。
...
- 同じ run の再編集で既存の単位ファイル（`状態` / `After:`）をテンプレートで上書きしない
- 単位ファイルの計画テンプレートにプレースホルダー付き `After:` を置かない
