...
- **フェーズ3 / 4**: PixiJS 移行ゲートの管理。MVP3/4 時点では粒数・ズーム階層が増えていないため DOM/SVG を継続するが、破綻し始めたら前倒しする。フェーズ4 のキャラ表情（💪😩😴🙂）も移植時にスプライト表現（疲れ顔/ガッツポーズ等）へ拡張する。
...
デイリーのベストは `meta.dailyRuns` に保存され Title / RunResult には表示されるが、業界ランキングビュー（`IndustryScreen` / MVP5）へ「自分のデイリー記録」を差し込む擬似リーダーボードは未着手。シーズン/リーグ/デイリーラン（同一シード）を `state/meta`（第17章）へ恒久反映するかを判断する（現状はラン内の集約から都度生成）。
