...
| [19.1](../SPEC.md#191-面白さの定義と判定基準) | 面白さの定義と判定基準（F-1〜F-12） | プレイテストの合否判断基準。[playtest-findings.md](./playtest-findings.md) | 🟡 定義済み。F-1・F-2・F-4・F-5・F-7・F-8・F-10・F-11 が未充足でRI-73〜87。F-6 は RI-82 で敗北画面の次の一手を実装済み。F-12 は RI-81 でドラフトマリガンを実装済み。F-9 の「打てた手」観測は RI-89 で計測可能（差の再計測は playtest:report 参照）。F-3 は未検証 |
...
| Q1 で進化ツリーを取り切れ、ビルドの方向という概念が成立しない | 第11／第19.1 F-11 | [RI-86](./remaining-issues.md#ri-86-q1-で進化ツリーを取り切れてしまいビルドの方向という概念が成立しない) 進行中（希少性は導入済） |
