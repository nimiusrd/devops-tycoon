...
- **解放セットは `startRun` 時に最新 `meta` から再計算して渡す**。`createGame` は `createRunEngine` を一度だけ呼び（`src/game.ts:95`）、以降の `startRun` は同じ engine インスタンスを再利用する（`src/game.ts:142-145`）。そのため constructor 初期値（`createRunEngine({ ..., unlocked })`）**だけ**で解放セットを固定すると、7c のメタショップ購入後の次ランに反映されず、ページ再読み込みまで古いプールのままになる。**constructor 専用は採らず**、`startRun`（および `newRun` / `startDailyRun`）の中で `unlockedContent(loadMeta())` を解決して engine に渡す（または engine の解放セットを明示更新する）形に限定する。`buildShop` のカード抽選・`offerRelic` のレリックプールも、その時点の解放セットでフィルタする。
- 解放セットは**ラン開始時に確定し、ラン中は固定**（ラン内で解放しても次ランから反映）にして決定論を保つ。購入→次ラン反映（DoD）と決定論の両立は「次ランの `startRun` で最新 meta を読む」で成立する。
