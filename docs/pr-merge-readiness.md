...
PR の準備状況は [独立 Action v0.2.0](https://github.com/nimiusrd/pr-merge-readiness-action/releases/tag/v0.2.0) により観測します。固定 SHA は `68a4aa9a0b4778452fc7f74d77c48d508d62bfe2` です。参考 Check の名前は `Autonomous Merge Shadow / PR #番号`、conclusion は常に `neutral` です。
...
設定を変更したら、固定 SHA の Action を取得し、uv 0.12.13 の `uv python install --no-config 3.14` で Python を用意します。このリポジトリのルートから `bash /path/to/action/run.sh generate-workflow --config .github/pr-merge-readiness.toml --output .github/workflows/pr-merge-readiness.yml` を実行してください。生成物の手編集は避け、共通検証 workflow の `--check` で一致を確認します。
