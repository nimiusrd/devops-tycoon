...
function judgeF9Sets(complete) {
  ...
  return {
    distinctEffectiveSetCount: new Set(setKeys).size,
...
export function evaluateF8F9(loaded, options = {}) {
  ...
  const distinctEffectiveSetCount = Math.min(...strata.map((s) => s.distinctEffectiveSetCount));
  const f9Pass = distinctEffectiveSetCount >= F9_MIN_DISTINCT_SETS;
  const qualifyingReasons = [...new Set(strata.flatMap((s) => s.qualifyingReasons))].sort();
  const byReason = {};
