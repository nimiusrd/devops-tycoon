...
`@codex review` を投稿したあとは、Codex の応答を待つ。指摘があればコメントし、なければそのコメントに 👍 する。

指摘が出たら対応して再度 `@codex review` する。最新の `@codex review` に 👍 が付き、未解決スレッドもなく、CI も問題なければ merge-ready として報告して止める。明示がない限りマージしない。
