...
- 現行実装では、AI依存度が高く、AIリテラシーや品質が低いとReworkが増える。
...
ここで、`aiDependency`と`AI利用時`は異なる粒度の状態である。
...
`reworkAttempts`は通常Reworkだけでなく、Incidentで点火した場合にも1増える。そのため、どちらかを経験するたびに、その後の通常Rework確率を半減させる。

`MAX_REWORK=3`は通常Rework判定だけの上限である。Incident判定は通常Reworkより先に毎回行われ、`reworkAttempts`が3以上でも発生しうる。したがって「タスクのIncidentとReworkが合計3回で必ず収束する」という保証ではない。

`pRework`はIncidentが起きなかった場合の条件付き確率なので、Review1回あたりの無条件Rework確率は概ね次の値になる。
...
現行式は、AI依存度を「AI利用によって組織全体へ蓄積したReworkリスク」として扱う。そのため、AI依存度による傾きはAI支援あり・なしで等しく、AI支援ありには常にタスク固有の追加リスクがある。

一方、「AI依存度が高い組織は、AIなしでは品質を保って仕事をしにくい」という意味を持たせるなら、AI支援なしのタスクへ依存度ペナルティを強く掛ける方が自然である。現行の`aiDependency`は、次の異なる概念を一つの値へ混在させている。

| 概念 | 影響先 | 推奨する表現 |
| --- | --- | --- |
| AI生成コードや工程上の負債 | AI支援の有無にかかわらず全タスク | `quality`、`techDebt`、`testCoverage`などの共有状態 |
| AIなしでは通常の仕事を維持できない依存 | 主にAI支援なしのタスク | `aiDependency`と手作業能力の相互作用 |
| AIなしで品質を保つ能力 | AI支援なしのタスク | `manualCapability`などの中間変数 |
| AIを安全に使い出力を検証する能力 | AI支援ありのタスク | 組織の`aiLiteracy`、個人の`aiMastery` |

再設計する場合は、全値を`[0, 1]`へ正規化し、次の変数を明示的に分ける。

| 記号 | 意味 |
| --- | --- |
| `U` | 対象タスクがAI支援ありなら`1`、なしなら`0` |
| `D` | チームのAI依存度 |
| `C` | チームが保持する手作業能力`manualCapability` |
| `I` | 稼働コーダーの平均実装力 |
| `G` | ドキュメント充実度 |
| `A_member` | AIを配った稼働コーダーの平均AI習熟度 |
| `L` | 組織のAIリテラシー |
| `Q` | 品質 |
| `T` | `clamp(techDebt / TECH_DEBT_CAP, 0, 1)` |

タスクへ適用する実効手作業能力`M`と実効AI習熟`A`は、初期案ではチーム集約値として算出する。
粗粒度チームの`techDebt`は`TECH_DEBT_CAP`を超えて増えうるため、`T`は明示的に`[0, 1]`へ収め、負債項の最大寄与を係数どおりに保つ。

```text
M = clamp(0.50 × C + 0.30 × I + 0.20 × G, 0, 1)
A = clamp(0.60 × A_member + 0.40 × L, 0, 1)
```

`C`はAI利用による能力保持の変化、`I`はメンバーが元から持つ実装力、`G`は属人知識を補うドキュメントを表す。`A`は個人のAI利用能力と組織の利用規律を合成する。

#### 4.5.2 Rework候補式

次を調整開始点となる候補式とする。係数は意味と曲線形状を検討するための仮値であり、SSoTへの値移動とは別のモデル変更PRでMonte Carlo検証して確定する。
...
- `sharedRisk`: AI支援の有無によらず、品質と技術的負債から受ける共通リスク
- `manualRisk`: 手作業能力が低いほど増え、AI依存度との相互作用で傾きが大きくなるリスク
- `aiRisk`: AI習熟が低いほど増え、依存度が高いと過信・検証不足が増幅するリスク

`manualRisk`の依存度係数を`aiRisk`より大きくすることで、低依存時はAI支援なしの方が安全でも、高依存・低手作業能力ではAI支援なしの方が危険になる交差を表現する。AI習熟が十分に高ければ、高依存でもAI支援タスクのリスク上昇を抑えられる。

仮に`M=0.60`、`A=0.50`、`Q=0.60`、`T=0.20`、初回Review、その他の補正なしとすると、候補式の条件付きRework確率は次のようになる。

| AI依存度`D` | AI支援なし | AI支援あり |
| ---: | ---: | ---: |
| 0% | 11.6% | 13.4% |
| 25% | 14.4% | 15.2% |
| 50% | 17.2% | 16.9% |
...
この条件では依存度約43%で2本の線が交差する。交差点そのものを固定目標にはせず、手作業能力とAI習熟の構成によって前後することを意図する。

![AI依存度によってAI支援ありとなしのRework確率が交差する候補曲線](./assets/proposed-ai-dependency-curves.svg)

グラフは候補式の形状を確認するための仮係数による計算結果であり、現行ゲームの挙動や確定済みの目標値ではない。

`A_member`を新しい`aiRisk`へ組み込む場合、現在の編成集約が同じAI習熟を`Rework加算補正`へ反映している処理は外す。同じ能力を`aiRisk`と補正値の両方へ入れる二重計上を避けるためである。

#### 4.5.3 状態の更新候補

AI依存度と手作業能力は相関するが、同じ状態ではない。AI依存度はツールなしで工程を維持できるか、手作業能力は人が自力で品質を保てるかを表し、異なる速度で更新する。

```text
R = clamp(0.50 × G + 0.50 × L, 0, 1)

D_next =
  clamp(
    D
    + U × dependencyGain × (1 - R)
    - (1 - U) × independenceRecovery
    - 依存軽減施策,
    0,
    1
  )
...
C_next =
  clamp(
    C
    - U × capabilityDecay × (1 - G)
    + (1 - U) × practiceRecovery × (1 - C)
    + 研修・ペア作業による回復,
    0,
    1
  )
...
- ドキュメントとAIリテラシーが高いほど、AI利用がそのまま依存へ変わりにくい。
- AIなしの実装は依存を緩和し、失った手作業能力を徐々に回復させる。
- ドキュメントが高いほど、AI利用中の手作業能力低下を抑える。
- 研修、ペア作業、AI一時停止などへ、状態を回復する明確な役割を与えられる。

更新係数は1タスクの選択で状態が大きく往復しない値にする。AI依存度は比較的速く、手作業能力は複数スプリントで緩やかに変化する時間尺度を初期仮説とする。

#### 4.5.4 検証する不変条件

候補式を実装する場合は、少なくとも次を検証する。

- 品質が高いほど、AI支援の有無によらずRework確率が下がる。
- 技術的負債が高いほど、AI支援の有無によらずRework確率が上がる。
- 手作業能力はAI支援なしのリスクだけを下げる。
- AI習熟はAI支援ありのリスクだけを下げる。
- AI依存度が高いほど両方のリスクは上がりうるが、上昇幅は能力構成で変わる。
- 低依存ではAI支援なし、高依存・低手作業能力ではAI支援ありが安全になる構成が存在する。
...
- 現行の`aiDependency`は、共有コードのリスク、AIなしで仕事をする能力、AI出力を検証する能力を分離していない。
...
11. AI依存度を変更するときは、共有リスク、手作業能力、AI習熟のどれを表現する変更かを区別する。
