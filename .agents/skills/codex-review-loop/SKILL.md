...
👀 だけが付いている間は未完了。👍 を待ち続けず、指摘なしコメントが来たら完了として先へ進む。指摘が出たら対応して再度 `@codex review` する。コード変更なしで dismiss して resolve しただけの場合は、同じ HEAD でも再依頼してよい。
...
- PR がマージ可能（競合なし）
- 未解決スレッドがない
- 現在の PR HEAD に対する必須 CI が完了している（成功、または許容された skip。pending / queued、および新 HEAD のチェック未作成は待機）
