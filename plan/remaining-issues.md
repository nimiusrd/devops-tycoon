...
| ID | 項目 | 優先度 | 状態 | 依存 | 関連 |
| --- | --- | --- | --- | --- | --- |
| RI-43 | 効果タグ自動生成基盤＋ビート選択肢のリスク・リターン表示 | 高 | 未着手 | — | 第9 / 18.2 |
...
| ID | 項目 | 優先度 | 状態 | 依存 | 関連 |
| --- | --- | --- | --- | --- | --- |
| RI-35 | 介入アクションのテーブル駆動テスト | 中 | 未着手 | — | 第6 |
...
#### RI-43 効果タグ自動生成基盤＋ビート選択肢のリスク・リターン表示 — 優先度:高

- **現状**: `BeatScreen.tsx` は選択肢の `label` と散文 `description` を表示するだけ。効果数値は
  `src/data/events.ts` の `EventOutcome`（`delivered` / `morale` / `seniorHp` / `techDebt` / `budget` /
  `quality` / `trust` 等の固定デルタ）としてデータに存在するのに、説明文は**別管理の手書き文字列**で、
  バランス調整時に実効果と表示が乖離しうる。
- **やること**: 効果データ（`EventOutcome` を起点に、後続 RI で `CardEffects` / `OrgAdjust` 等へ拡張）から
  「出荷 +30 / 士気 −15」形式の**色分けタグ（プラス=緑 / マイナス=赤）を自動生成する共通フォーマッタ**を
  新設し、まずビート選択肢に適用する。選ぶ前にリスク・リターンが一目でわかる状態にする。
  判定系イベント（1択）には結果適用時に「何が起きたか」のデルタタグを出す。
- **合わせて**: `description` に埋め込まれた数値の二重管理を解消し、数値部分は実データ由来に寄せる
  （散文はフレーバーに専念させる）。
- **規模感**: 小〜中。フォーマッタは純関数として `render/` か `ui/` 配下に置き、Vitest で
  データ→表示文字列の対応を検証する。
