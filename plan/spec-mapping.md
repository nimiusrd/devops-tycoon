...
| [17](../SPEC.md#17-メタ進行とアンロック) | メタ進行とアンロック | `src/state/meta.ts`, `src/state/metaPersistence.ts`, `src/data/unlocks.ts`, `src/ui/MetaShopScreen.tsx`, `AchievementCollectionScreen.tsx` / `tests/unit/meta.test.ts`, `metaPersistence.test.ts`, `meta-unlock-run.test.ts`, `unlocks.test.ts` | 🟡 IndexedDB によるカード/レリックの永続解放・旧 localStorage 移行・メタショップ・実績閲覧は実装。SPEC §17 の「開始時の組織（プリセット）解放」は未実装（`UnlockKind = 'card' \| 'relic'`、[remaining-issues.md](./remaining-issues.md) RI-25） |
