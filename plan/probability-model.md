...
再設計する場合の概念式は次のとおりとする。係数と具体的な合成方法は、SSoTへの値移動とは別のモデル変更として決定する。

```text
sharedRisk =
  f(quality, techDebt, testCoverage, documentation)
...
aiRisk =
  AI支援あり
  × f(aiMastery, aiLiteracy, aiDependency)

pRework =
  clamp(
    baseRisk + sharedRisk + manualRisk + aiRisk + その他の補正,
    min,
    max
  )
```

この形では、依存度が低いときはAI支援なしの方が安全でも、依存度が高く手作業能力が低いとAI支援なしの方が危険になる。2本の曲線は条件によって交差しうる。
