...
| [18](../SPEC.md#18-視覚表現) | Pixi描画、演出、音響 | `src/render/adapters/`, `src/render/boardEffects.ts`, `src/ui/*Effects.tsx`, `src/audio/` | ✅ 中核のPixi盤面・DOM演出・音響、Review の流れ・滞留・熱、炎上・鎮火・介入リアクションと常駐オーラの GPU 演出、描画予算・DOM/Pixi同等性・回帰契約（RI-140〜143）を実装済み。HUD等の離散UIはDOMのまま維持する |
