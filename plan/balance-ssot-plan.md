...
本書はRI-105で対象境界と互換方針を確定した実装契約である。型付きレジストリ、生成パラメータ表、工程モデル、メンバー・採用、介入・差配、ラン進行・経済、KPI・勝敗・診断、粗粒度チーム、ペーシング、カード実行ルール、タスク分布、メタ進行・デイリー条件、代表確率曲線は移行済み（RI-106〜114、RI-120、RI-122〜124）。残る領域はルールセットのデイリー伝播である。実装の親エピックと1PR単位のバックログは[RI-104](./remaining-issues.md#ri-104-バランスパラメータssotの導入)で追跡する。
