...
| RI-60 | 初見向けオンボーディング(チュートリアル/ヘルプ) | 中 | 未着手 | — | 第4 / 6 / 7 |
| RI-62 | ゲームスピードの調整(スプリントが速すぎて介入できない) | 高 | 未着手 | — | 第3.1 / 6 |
...
| RI-58 | ラン途中セーブと復帰(ランステート永続化) | 中 | 未着手 | — | 第17 / 22 |
| RI-61 | リプレイ保存・閲覧 | 低 | 保留(要判断) | RI-58 | 第22 / 23 |
...
| RI-24 | メタ解放対象の拡張(メンバー/トレイト/初期カード等) | 低 | 完了 | — | 第17 |
| RI-25 | 開始時の組織プリセット(実装 or 足場削除) | 低 | 完了 | — | 第17 |
| RI-26 | 採用の入口拡張(専用ノード/イベント/ショップ枠) | 低 | 完了 | — | 第4 |
| RI-27 | 個体メンバーの集約粒度を深める | 低 | 完了 | — | 第4.7–4.9 |
| RI-28 | 四半期レビュー評価→メタ報酬の接続（′可視化 / ″解放ゲート完了） | 低 | 完了 | — | 第8 / 17 |
...
#### RI-09 アクションバーのマネージャー像 — 優先度:低 / 完了

**完了**: 旧モック main-screen footer（`.focus .mgr`）のマネージャー SVG を
...
#### RI-60 初見向けオンボーディング(チュートリアル/ヘルプ) — 優先度:中 / 未着手

効果タグ（RI-43〜45）・介入 8 種＋武装ドラッグ（RI-30）・手札発動・ビート判定/選択・四半期レビューと
システム量が増え、初見プレイヤーが「最初のスプリントで何をすべきか」に到達しづらくなっている。
SPEC に明記はないが、プレイテスト所感 2 回（可視化不足→フィードバック不足）の次に来る導線課題として
洗い出した。候補: 初回ラン限定の段階的ガイド（介入バー→レビュー渋滞→コンボの順にハイライト）、
タイトルからの「遊び方」ヘルプ画面、`?tutorial=` クエリでの E2E 検証フック。
論点: ガイドは sim 決定論の外側（UI 層のみ）に置くこと、既プレイヤーへの非表示条件
（表示済みフラグを `MetaState` へ永続化）、世界観の制約（第2.1）内のトーンで書くこと。

#### RI-62 ゲームスピードの調整(スプリントが速すぎて介入できない) — 優先度:高 / 未着手

**課題**（プレイテスト所感 2026-07 第3回）: スプリントの自動進行が速すぎ、プレイヤーが介入する前に
終わってしまう。実測（2026-07-18、normal・代表 6 seed × 各3スプリント）では、`useRun` が実時間
60ms ごとに 1 tick（`FRAME_MS=60` / `SIM_STEP_MS=100`）進めるため、スプリント（36〜204 tick、
平均 104 tick）は**実時間 2.2〜12.2 秒（中央値 約6秒）で完了**する。集中力 12・手札 3 枚・
クールダウン持ちの介入 8 種を使い切る時間はなく、時限モディファイア（30〜40 tick）も実時間
1.8〜2.4 秒しか持たない。RI-49〜55 で整備した介入フィードバックループが、そもそも介入する
時間的余地がないために活きていない。

**対応方針**（①のテンポ減速**単独では §3.1 を満たせない**。**①＋④の併用を必須の主案**とし、
②の速度コントロールをセットで提供する。③は任意の上積み）:

1. **既定テンポの減速（④と必ず併用）**: `useRun` の実時間→sim 時間の比を変える。ただし単一の
   tick 実時間では現状の tick 数分布（36〜204 tick・平均 104 tick）のまま §3.1 を満たせない——
   最短 36 tick を「30 秒以上」にするには **833ms/tick 以上**、最長 204 tick を「120 秒以内」に
   収めるには **588ms/tick 以下**が必要で、両立しない。よって①は分布の中心を目安帯へ寄せる
   役割（例: 平均 104 tick × 600〜1150ms ≒ 60〜120 秒）に留め、分布の端は④のスプリント長調整で
   担保する。減速自体は sim 層・seed 決定論・モンテカルロ（tick 数ベース）には非干渉で、
   E2E / gallery も `step()` 直接駆動のため影響なし（要確認）。1 tick が 1 秒近くなる場合の
   粒のカクつきも、④で tick を細かく刻み直して吸収する。
2. **速度コントロール UI**: 一時停止 / 1x / 2x のトグルをスプリント画面に追加。`pause()` 基盤は
   実装済みだが、E2E・ボススローモ（RI-10 の `pauseBriefly`）と epoch 所有権の衝突回避が必要。
3. **重要イベントの自動ポーズ**: 点火・Review 渋滞しきい値・ボス Incident で短い自動ポーズ＋
   ハイライト（アンドン的な「止めて見せる」。RI-10 のスローモ基盤を再利用）。
4. **スプリント長の調整（必須。tick 数の下限引き上げ＋分布幅の圧縮）**: `taskCount` / intake
   ペースの変更で最短スプリントの tick 数を引き上げ、36〜204 tick の分布幅を圧縮する。①の
   単一テンポで §3.1 の上下限（最短 30 秒以上・120 秒以内）を同時に満たすにはこれが前提となる
   **必須条件**。バランス全体・MC 許容レンジ・終端 seed（E2E_TERMINAL_*）に波及するため
   コストは大きいが、DoD 達成には避けられない。

**論点**: 減速後の「待ち時間の間延び」対策（2x トグルとセットで入れる）、時限モディファイアの
体感持続時間の再調整、`FRAME_MS` / `SIM_STEP_MS` の意味の再整理（現状 sim 時間は実時間の約1.67倍速）。

**目標値**: SPEC **§3.1「時間の目安（ペーシング規定）」**として規定済み（本課題を受けて新設）。
1x 基準でスプリント 60〜120 秒（最短 30 秒以上）・ボス 90〜180 秒・時限効果は体感 10 秒以上・
一時停止と 2x 倍速を標準提供、など。現状実測（中央値 約6秒）との乖離が大きく、
実装時はこの表を DoD として検証する（体感時間は E2E ではなく実測計算＝tick 数×テンポ設定で確認できる）。
...
再読み込みを検証する。リプレイ・大量履歴の保存設計は引き続き非スコープ（→ RI-61 へ切り出し）。

#### RI-58 ラン途中セーブと復帰(ランステート永続化) — 優先度:中 / 未着手

メタ進行は IndexedDB へ永続化済み（RI-57）だが、ラン本体は揮発でリロード/タブ閉じにより消える。
1 ランが四半期×スプリントで長いブラウザゲームなので、中断復帰は体験の底上げに直結する
（SPEC §22 の保存想定用途にも「セーブ」が含まれる）。論点: 保存形式（`RunEngine` / `SprintState` の
スナップショット直列化 vs seed＋入力ログからのリプレイ再構築。後者は RI-61 と土台を共有）、
保存タイミング（フェーズ遷移時のみ保存しスプリント tick 中は保存しない＝セーブスカム抑制）、
スキーマバージョニングと非互換時の切り捨て方針（RI-57 の既定値補完と同じ流儀）、
タイトル画面への「続きから」導線と E2E（リロード復帰）検証。

#### RI-61 リプレイ保存・閲覧 — 優先度:低 / 保留(要判断)（依存: RI-58）

SPEC §22 の保存想定用途「リプレイ」と §23 の「レビュー地獄リプレイ」「なぜ燃えたか解説ログ」に
またがる繰り越しで、RI-57 が「リプレイ・大量履歴の保存設計は非スコープ」と明言した分の受け皿。
seed 決定論＋介入ログ（`SprintEvent` / `actionCounts`）により理論上は再生可能な土台があるが、
閲覧 UI・保存容量・バージョン互換の維持コストが大きい。RI-58 の保存形式判断とセットで、
実装するか §23 拡張（RI-34）として保留し続けるかを判断する。
...
#### RI-18 メタ解放コスト・points 配分の許容レンジ — 優先度:中（依存: RI-14）/ 完了

**完了**: `tests/unit/helpers/metaRewardRanges.ts` に解放コスト帯（暫定 25〜50 を余裕込みで包む
20〜60）と勝利/敗北 points 報酬の許容レンジを定義し、`tests/unit/unlocks.test.ts` で
`UNLOCK_DEFS` の ID/contentId 重複なし・コスト帯・card/relic 両立を検証。
`tests/unit/monteCarlo.test.ts` の RI-18 節で normal 難易度の代表 8 seed（`ri18-meta-0..7`）を
通しプレイし、points 報酬レンジ（勝利 18〜35 / 敗北 4〜12・`scoreMul`=1）と「勝利 1 ランでは
最安解放に届かず数ランで到達」「全解放は 5〜50 ランで到達可能」なペース配分を回帰検知する。
現行 `src/data/unlocks.ts` / `applyRunReward` の暫定値は調整不要と判断した。
...
#### RI-24 メタ解放対象の拡張 — 優先度:低 / 完了

**完了（スコープ外）**: メタ解放の対象をカード／レリックのドラフト・ショッププール拡張に限定し、
開始キット（初期カード／初期レリック）・メンバー／トレイト解放（初期ロスター差し替え・採用プール
拡張含む）・追加イベント／追加試練のメタ解放は正式実装しない。開始組織は難易度（第16章）が担い、
周回の選択肢拡大は既存のカード／レリック解放で足りる。開始キットやロスター解放は RI-25 プリセットと
同種の役割重複を生む。現行実装（`UnlockKind = 'card' | 'relic'`、`UnlockedContent` の cards/relics
のみ）が既にその範囲のため、コード変更は不要。SPEC §17 に境界を明記した。

#### RI-25 開始時の組織プリセット — 優先度:低 / 完了

**完了（スコープ外）**: 定義＋タイトル選択 UI＋`startRun` 引数の 3 点セットを伴わないなら
スコープ外、という合意に従い正式実装しない。難易度（第16章）が開始組織の選定を既に担うため、
メタ解放プリセットは役割重複が大きい。到達不能だった `MetaState.unlockedPresets` /
`UnlockedContent.presets` / `purchaseUnlock`・`MetaShopScreen` の preset 分岐を削除し、
SPEC §17 からも当該文言を外した。開始キット（初期カード／ロスター等）は RI-24 でスコープ外確定。

#### RI-26 採用の入口拡張 — 優先度:低 / 完了

**完了**: 個体採用の入口を休息に加え 3 系統へ拡張した。
- ショップ採用枠: `ShopOffer.recruit` / `buyShopRecruit` / `ShopScreen`
- 専用採用ビート: `recruit-offer` → `phase:'recruit'` / `RecruitScreen` / `recruitChoose`
- イベント即時採用: `EventOutcome.grantRecruit`（`urgent-hire`）
共通コアは `RunEngine.tryRecruit`（`RECRUIT_COST=25`、空き枠ゲート、入口別 RNG）。
Vitest でショップ/フェーズ/イベント経路を検証済み。

#### RI-27 個体メンバーの集約粒度を深める — 優先度:低 / 完了

**完了**: 個体名・カードは全社/部署に出さず、稼働人数（休職除外）・島アバター数（1〜4）・
AIボット（`aiAssignedCount`）・島バッジ/部署バナーの人数表示へロスターを接続した。
`activeEngineerCount` / `aiAssignedCount` → `OrgScaleInput` → `Team` → `OrgTeamActor` /
`planOrgBoardScene` / `planDeptBoardScene`。Vitest で集約ヘルパー・生成・ラン統合を検証済み。

#### RI-28 四半期レビュー評価→メタ報酬の接続 — 優先度:低 / 完了

**完了（コア + RI-28′ + RI-28″）**: `reviewHistory` の outcome を `applyRunReward` へ接続済み
（敗北時 `missed_adjustable` → 学習ボーナス、勝利時 `exceeded`/`met` → レビューボーナス）。
RI-28′ で `computeRunRewardBreakdown` の内訳を `RunResultScreen` に表示
（基本 / 敗北学習 / 超過達成・達成）。永続スキーマ変更なし。

RI-28″ では outcome を解放条件ゲートに接続した（ラン中即時ポイント付与は採らず）。
勝利時に `review-exceeded`（`exceeded` 経験）/ `review-survivor`（`missed_adjustable` 経験）を実績付与し、
`unlock-devin` / `unlock-hire-senior` の `requires` に紐づける。
...
| 横断テーマ F（メタ解放対象の拡張） | RI-24（スコープ外完了） |
