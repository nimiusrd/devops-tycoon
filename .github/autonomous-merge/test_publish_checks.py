...
ARTIFACT = "shadow-observations-42-1"
...
class FixtureAPI:
    ...
    def __init__(self):
        self.prs = {
            n: {"number": n, "head": {"sha": HEAD}, "base": {"sha": BASE, "ref": "main"},
                "state": "open", "draft": False, "updated_at": AT, "merged_at": None,
                "merge_commit_sha": MERGE}
            for n in (1, 2)
        }
        self.checks = []
        self.writes = []
        self.run = {"id": 5, "status": "in_progress", "run_attempt": 2,
                    "run_started_at": LATER, "event": "pull_request", "head_sha": MERGE}
        self.drift = None
        self.reads = 0
        self.failure = None
    ...
    def request(self, path, body=None, method=None):
        ...
        if body is not None:
            ...
            if method == "PATCH":
                ...
                self.checks.append({**deepcopy(body), "id": len(self.checks) + 1,
                                    "app": {"id": 15368}})
            return {}
        if "/pulls/" in path:
            self.reads += 1
...
class CheckTests(unittest.TestCase):
    def test_four_decisions_are_neutral_and_link_to_exact_artifact(self):
        for decision in ("SHADOW_CONDITIONS_MET", "WAITING", "HUMAN_REVIEW_REQUIRED", "INSUFFICIENT_DATA"):
            with self.subTest(decision=decision):
                ...
                body = api.writes[0][1]
                self.assertEqual((body["head_sha"], body["status"], body["conclusion"]),
    ...
    def test_base_draft_updated_at_and_closed_changes_require_reobservation(self):
        for change in ({"base": {"sha": "d" * 40, "ref": "other"}},
                       {"draft": True}, {"updated_at": LATER}, {"state": "closed"}):
            api = FixtureAPI()
    ...
    def test_old_head_event_and_title_only_edit_are_ignored(self):
        ...
        payload = deepcopy(api.prs[1])
        api.prs[1]["head"]["sha"] = "d" * 40
        self.assertEqual(mark_event(api, {"pull_request": payload, "action": "synchronize"}, URL), [])
        self.assertEqual(mark_event(api, {"pull_request": api.prs[1], "action": "edited",
                                          "changes": {"title": {"from": "old"}}}, URL), [])
        self.assertFalse(api.writes)

    def test_ci_start_targets_current_head_merge_or_push_base_only(self):
        for event, head, count in (("pull_request", MERGE, 2), ("pull_request", HEAD, 2),
                                   ("push", BASE, 2), ("pull_request", "e" * 40, 0)):
            api = FixtureAPI()
    ...
    def test_manifest_rejects_other_attempt_and_missing_or_global_failure(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            api = FixtureAPI()
            with self.assertRaises(OSError):
                publish_directory(api, root, "42", "1", URL, ARTIFACT)
            for change in ({"run_attempt": "2"}, {"repository": "other/repo"},
                           {"collection_failed": True}):
                manifest = {"version": 1, "repository": api.repository, "run_id": "42",
                            "run_attempt": "1", "reports": [], "collection_failed": False, **change}
                (root / "manifest.json").write_text(json.dumps(manifest))
                with self.assertRaises(PublishError):
                    publish_directory(api, root, "42", "1", URL, ARTIFACT)
            self.assertFalse(api.writes)

    def test_manifest_ignores_unlisted_files_and_continues_after_missing_report(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            api = FixtureAPI()
            manifest = {"version": 1, "repository": api.repository, "run_id": "42",
                        "run_attempt": "1", "reports": [1, 2], "collection_failed": False}
            (root / "manifest.json").write_text(json.dumps(manifest))
            (root / "pr-2.json").write_text(json.dumps(report(2)))
            (root / "pr-999.json").write_text("invalid old file")
            results = publish_directory(api, root, "42", "1", URL, ARTIFACT)
            self.assertIn("error", results[0])
            self.assertEqual(results[1]["result"], "published")
            self.assertEqual(api.checks[0]["name"], CHECK_PREFIX + "2")

    def test_cli_requires_current_attempt_and_returns_failure_for_missing_report(self):
        with tempfile.TemporaryDirectory() as directory:
            args = ["publish_checks.py", "--repository", "example/project", "--report-dir", directory,
                    "--artifact-name", "shadow-observations-42-1"]
            with patch("sys.argv", args), patch.dict(os.environ, {"GITHUB_RUN_ID": "42", "GITHUB_RUN_ATTEMPT": "2"}):
