...
async function openExactSpend(page: Page): Promise<SpendTarget> {
  await page.goto('/?seed=spend-risk&renderer=pixi');
  await expect(page.getByTestId('title')).toBeVisible();
  const found = await page.evaluate((recruitCost) => {
      ...
      while (guard < 80) {
        ...
        if (state.phase === 'beat') {
          ...
          if (eventId && interesting.has(eventId)) {
            ...
            if (opened.phase === 'shop' && opened.shop) {
            ...
            if (opened.phase === 'recruit' && opened.budget === recruitCost) {
              return {
                seed,
            ...
            if (opened.phase === 'rest' && opened.budget === recruitCost) {
              return {
                seed,
  ...
  expect(found, '予算ちょうどでの採用か購入に到達できる').not.toBeNull();
  return found!;
}
...
test.describe('予算枯渇になる購入と採用', () => {
  ...
  test('確定前はランが終わらず、320px でも取消と確定に届く', async ({ page }) => {
    const target = await openExactSpend(page);
