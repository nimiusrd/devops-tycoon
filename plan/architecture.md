...
- 初期段階: DOM/SVG（モックアップ準拠）で素早く。周回・診断・勝敗の通しプレイを優先し、過剰投資を避ける。ただし sim は最初から分離・seed付き。
- 巨大組織・4階層ズーム: 4階層ズーム・巨大組織ビューの**描画非依存の基礎**（アイソメ投影 / 深度ソート / 画面外カリング / スプライトプール）を `src/render/iso.ts` に純TS で実装し、数値検証した（第22.5）。全社/部署/業界ビューは既定では DOM/SVG（Framer Motion でクロスフェード）で実装し、`iso.ts` の座標系を共有する。`?renderer=pixi` の opt-in で全社マップ（pixi-viewport）・部署ビュー・現場盤面を PixiJS 描画へ差し替え可能（RI-11）。実ピクセル/WebGL 検証は opt-in Playwright（`npm run test:e2e:pixi`）に集約し、CI 既定 job では実 WebGL を回さない。React/TS/Framer Motion/Recharts は役割を限定して継続。
