...
- `{N}` は既存 `RI-(\d+)` の最大+1（欠番再利用なし）。フルシャード run の再ベースラインごとに新規エピック。
...
エピック用トラッキング Issue には run URL・`headSha`・方針を置く。実装単位は **GitHub のサブイシュー**としてエピック配下にぶら下げる（推奨）。本文に `#123` リンクを列挙してもよい。  
子の open / closed はサブイシュー一覧、または本文リンクの GitHub 表示で識別する（別途状態表は作らない）。  
子がすべて closed になったらエピック Issue も close し、`remaining-issues.md` を完了にする。  
再ベースラインで未消化を引き継ぐときは、旧エピック Issue と引き継いだ旧子 Issue を置換理由付きで close する。
