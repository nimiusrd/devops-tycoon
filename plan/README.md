...
- コアループは実装済み。優先度「高」の未着手課題は、オンボーディングとシニア燃え尽きの断絶（RI-67）、四半期レビュー Delivery KPI のスケール不整合（RI-68）など。ミューテーション改善の現行エピックは [RI-91](https://github.com/nimiusrd/devops-tycoon/issues/187)（run 30698016740）。採番・進捗の正本は GitHub Issue（本ディレクトリに番号予約しない）。一般バックログの全容は[remaining-issues.md](./remaining-issues.md)を参照。
