...
本書はRI-105で対象境界と互換方針を確定した実装契約である。型付きレジストリ、生成パラメータ表、工程モデル、メンバー・採用、介入・差配は移行済み（RI-106〜110）。残る領域は[`src/sim/`](../src/sim/)および[`src/data/`](../src/data/)の直書き値と照合して移す。実装の親エピックと1PR単位のバックログは[RI-104](./remaining-issues.md#ri-104-バランスパラメータssotの導入)で追跡する。
...
- メンバー・採用、介入・差配は移行済み（RI-109、RI-110）。ラン進行、KPI、勝敗・診断条件、ペーシング、メタ進行・デイリー条件を領域ごとに移す。
- 既存の`src/data/`定義をパラメータ一覧へ集約する。
- 粗粒度モデルの係数を移し、詳細モデルとの方向性を検証する。
- RI-123で移行済みの値と純粋な計算関数から代表確率曲線を生成し、手書きグラフを置き換える。

完了条件: メンバー・採用、介入・差配、カード実行ルールは充足済み（RI-109、RI-110、RI-122）。残るラン進行、KPI、粗粒度、ペーシング、コンテンツカタログ、メタ進行と代表曲線が未充足。調整対象として分類した値に安定ID、単位、説明が付き、代表確率曲線が同じ定義から生成されていること。
