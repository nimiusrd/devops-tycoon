...
- AI利用タスク1件につきAI依存度が`2.2`増える（Nightmare は難易度上書きで`1.35`。RI-74）
...
`reworkAttempts`は通常Reworkだけでなく、Incidentで点火した場合にも1増える。そのため、どちらかを経験するたびにクランプ前の`pBase`へ掛ける減衰係数が半分になる。ただし、`pRework`は最後に下限2%へクランプされるため、すでに下限へ達している場合の実確率は半減しない。
