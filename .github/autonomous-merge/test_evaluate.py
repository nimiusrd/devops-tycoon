...
class EvaluateTests(unittest.TestCase):
    ...
    def test_size_path_and_test_names_do_not_change_decision(self):
        for path in [
            ...
            with self.subTest(path=path):
                ...
                data["change"].update(
                    additions=100000, deletions=90000, changed_files=400
                )
    ...
    def test_current_merge_failure_is_not_hidden_by_head_success(self):
        data = facts()
        data["checks"].append(
            {**data["checks"][0], "id": 2, "sha": MERGE, "conclusion": "failure"}
        )
        self.assertEqual(self.decision(data), "HUMAN_REVIEW_REQUIRED")
    ...
    def test_review_submission_time_takes_precedence_over_draft_creation_id(self):
        ...
        self.assertEqual(
            self.decision(data, {**policy(), "minimum_approvals": 1}), "WAITING"
        )
    ...
    def test_cli_replays_recorded_facts_without_github(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(
                json.loads(run.stdout)["decision"], "SHADOW_CONDITIONS_MET"
            )

    def test_repository_policy_loads(self):
        self.assertEqual(
            load_policy(Path(__file__).with_name("policy.toml"))["version"], 2
        )
