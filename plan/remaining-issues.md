...
> 未着手の基本ループ再設計（ノード選択廃止→四半期トラック＋イベント判定）も本バックログの管理対象
> （**RI-33**）。残課題としては ID で追跡し、大きめの詳細設計は独立ファイル
...
| RI-05 | タスク粒の工程間移動アニメ | 中 | 未着手 | — | 第18.1 |
| RI-06 | 延焼の連鎖演出 | 中 | 未着手 | — | 第18.2 |
...
| RI-29 | リザルトの介入内訳表示(割り込み×N / 緊急対応×N) | 中 | 未着手 | — | 第4.6 |
...
| ID | 項目 | 優先度 | 状態 | 依存 | 関連 |
| --- | --- | --- | --- | --- | --- |
| RI-33 | ノード選択廃止→イベント判定化【エピック】 | 高 | 保留(要判断) | — | 第3 / 4.4 / 9.4 / 10 |
...
#### RI-29 リザルトの介入内訳表示 — 優先度:中

第4.6 例の「介入: 割り込み×3 / 緊急対応×1」の介入内訳が `SprintResult` に未集計で表示なし
（`summarizeSprint` が `interventionsUsed` / `focusSpent` を渡していない）。集計して表示する。
