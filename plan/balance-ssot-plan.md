...
本書はRI-105で対象境界と互換方針を確定した実装契約である。型付きレジストリ、生成パラメータ表、工程モデル、メンバー・採用、介入・差配、ラン進行・経済、KPI・勝敗・診断、粗粒度チーム、ペーシング、カード実行ルール、タスク分布、メタ進行・デイリー条件、代表確率曲線、デイリー・診断情報へのルールセット伝播、調整支援は移行済み（RI-106〜124）。親エピック[RI-104](./remaining-issues.md)は完了済みである。AI依存モデルの意味変更は[RI-134](./remaining-issues.md#ri-134-ai依存モデルの再設計)で扱う。
...
完了条件: 充足済み（RI-106、RI-107）。ゲーム挙動を変えず、SSoTからゲームとパラメータ表を生成できる。代表曲線はRI-123で追加済み。
...
完了条件: 工程モデルとタスク分布は充足済み（RI-108、RI-120）。代表確率曲線はRI-123で追加済み。移動だけでは結果を変えない。

### Phase 3: 周辺領域を移行

- メンバー・採用、介入・差配、ラン進行・経済、KPI・勝敗・診断条件は移行済み（RI-109〜RI-112）。ペーシングとメタ進行・デイリー条件も領域ごとに移した（RI-114、RI-124）。
- 既存の`src/data/`定義をパラメータ一覧へ集約する。
- 粗粒度モデルの係数を移し、詳細モデルとの方向性を検証する。
- RI-123で移行済みの値と純粋な計算関数から代表確率曲線を生成し、手書きグラフを置き換えた。

完了条件: 充足済み（RI-109〜RI-115、RI-120、RI-122〜RI-124）。調整対象として分類した値に安定ID、単位、説明が付き、代表確率曲線が同じ定義から生成されている。
...
完了条件: 充足済み（RI-116、RI-117、RI-118、RI-121）。バランス変更前後の結果をルールセット単位で識別できる。
...
完了条件: 充足済み（RI-119）。多数seedレポートとパラメータ差分は実装済み。開発用のバランス閲覧画面は任意であり、導入計画の対象外とする。
