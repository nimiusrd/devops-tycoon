...
API権限不足・取得中の変化は上記の実測でも確認しました。古い結果の到着、複数PRが同じheadを持つ場合、別attempt・artifact欠落、公開直前の変化などはPythonテストで補っています。残る実測は、次回09:43 JSTのscheduleイベントによる日次実行です。手動Labelsの成功を日次scheduleの成功として数えません。
